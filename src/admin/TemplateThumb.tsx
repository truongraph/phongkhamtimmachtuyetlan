...
function MiniSite({ t }: { t: SiteTemplate }) {
  ...
  const soft = shade(primary, 210), tint = shade(primary, 238), line = '#e6ebf1', slate = '#5b6b7f', ink = '#0f172a'
  ...
  const iconCss = (i: number): { box: any; fg: string } => {
    switch (t.icons) {
      ...
      case 'outline': return { box: { background: 'transparent', border: `1.5px solid ${shade(primary, 150)}` }, fg: primary }
  ...
  const sectionAlt = t.rhythm === 'plain' ? '#F6F8FB' : t.rhythm === 'tinted' ? shade(primary, 236) : tint
  const Eyebrow = () => {
  ...
  const Logo = ({ light }: { light?: boolean }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{ width: 40, height: 40, borderRadius: 11, background: light ? '#fff' : `linear-gradient(140deg,${primary},${navy})`, display: 'grid', placeItems: 'center' }}>
        <HeartPulse size={22} color={light ? primary : '#fff'} />
      </div>
      <div style={{ lineHeight: 1.15 }}>
        <div style={{ fontFamily: head, fontWeight: 700, fontSize: 19, color: light ? '#fff' : navy }}>{info.clinicName}</div>
  ...
  const Nav = ({ light }: { light?: boolean }) => (
    <div style={{ display: 'flex', gap: 22 }}>{NAV.map((n) => <span key={n} style={{ fontSize: 14, fontWeight: 600, color: light ? 'rgba(255,255,255,.9)' : navy }}>{n}</span>)}</div>
  )
  ...
  const Header = () => {
    if (t.header === 'utility') return (
      <div>
        <div style={{ background: navy, color: '#fff', fontSize: 11.5, padding: '7px 40px', display: 'flex', justifyContent: 'space-between' }}>
    ...
    if (t.header === 'bar') return (
      <div style={{ background: `linear-gradient(120deg,${primary},${navy})`, padding: '14px 40px', display: 'flex', alignItems: 'center' }}>
        <Logo light /><div style={{ flex: 1, display: 'flex', justifyContent: 'center' }}><Nav light /></div>
        <span style={{ fontSize: 13.5, fontWeight: 700, color: navy, background: '#fff', padding: '11px 18px', borderRadius: br === '999px' ? 999 : 10 }}>Đặt lịch khám</span>
      </div>
    )
  ...
  const Title = ({ center }: { center?: boolean }) => (
    <div style={{ textAlign: center ? 'center' : 'left', maxWidth: center ? 620 : 'none', margin: center ? '0 auto' : 0 }}>
      ...
      <div style={{ fontFamily: head, fontWeight: 700, fontSize: 40, lineHeight: 1.15, color: navy, marginTop: 14 }}>
  ...
  const DocImg = ({ w = 360 }: { w?: number }) => (
    <div style={{ position: 'relative', width: w, justifySelf: 'center' }}>
      <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', bottom: 0, width: '104%', height: '82%', borderRadius: imgRadius(t.image), background: `linear-gradient(160deg,${primary},${navy})` }} />
  ...
  const Form = () => (
    <div style={{ background: '#fff', borderRadius: 18, border: `1px solid ${line}`, overflow: 'hidden', boxShadow: '0 24px 50px rgba(11,59,112,.16)', width: 380, justifySelf: 'center' }}>
      <div style={{ background: `linear-gradient(120deg,${primary},${navy})`, color: '#fff', padding: '16px 20px', display: 'flex', gap: 12, alignItems: 'center' }}>
  ...
  const Hero = () => {
    ...
    return wrap(two(<div><Title /><div style={{ display: 'flex', gap: 12, marginTop: 22 }}>{content.stats.slice(0, 3).map((s) => <div key={s.id} style={{ flex: 1, background: '#fff', border: `1px solid ${line}`, borderRadius: 12, padding: '12px 14px' }}><div style={{ fontFamily: head, fontWeight: 700, fontSize: 20, color: navy }}>{s.value}</div><div style={{ fontSize: 12, color: slate }}>{s.label}</div></div>)}</div></div>, <DocImg />))
  }
  ...
  const Services = () => {
    const heading = <div style={{ marginBottom: 18 }}><Eyebrow /><div style={{ fontFamily: head, fontWeight: 700, fontSize: 24, color: navy, marginTop: 10 }}>{content.servicesTitle} <span style={{ color: accent }}>{content.servicesTitleHighlight}</span></div></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
          {content.services.slice(0, 4).map((svc, i) => (
            <div key={svc.id} style={{ ...cardStyle(), padding: 14, textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: head, fontWeight: 700, fontSize: 14, color: navy }}>{svc.title}</div>
            </div>
          ))}
        </div>
      )
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: 28 }}>
          {content.services.slice(0, 4).map((svc, i) => (
            <div key={svc.id} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '13px 0', borderBottom: `1px solid ${line}` }}>
              ...
              <div><div style={{ fontFamily: head, fontWeight: 700, fontSize: 15, color: navy }}>{svc.title}</div><div style={{ fontSize: 12.5, color: slate, marginTop: 2 }}>{svc.desc}</div></div>
            </div>
          ))}
        </div>
      )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {content.services.slice(0, 2).map((svc, i) => (
            <div key={svc.id} style={{ ...cardStyle(), display: 'flex', gap: 18, alignItems: 'center', flexDirection: i % 2 ? 'row-reverse' : 'row', padding: 18 }}>
              ...
              <div style={{ flex: 1, textAlign: i % 2 ? 'right' : 'left' }}><div style={{ fontFamily: head, fontWeight: 700, fontSize: 17, color: navy }}>{svc.title}</div><div style={{ fontSize: 13, color: slate, marginTop: 3 }}>{svc.desc}</div></div>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          {content.services.slice(0, 3).map((svc, i) => (
            <div key={svc.id} style={cardStyle()}>
              ...
              <div style={{ fontFamily: head, fontWeight: 700, fontSize: 16, color: navy, marginTop: 12 }}>{svc.title}</div>
